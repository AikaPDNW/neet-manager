...
from modules.gameState import time, encounter, rows, init_state
...
def maintable():
    ...
    for i in range(rows):
        ...
        tempBt.grid(row=i % 36, column=(i // 36) * 6, sticky=W, pady=2)
        actBt.append(tempBt)

        encounterLb = Label(master, textvariable=encounter[i])  # used textvariable instead
        encounterLb.grid(row=i % 36, column=(i // 36) * 6 + 1, sticky=W, pady=2)

        timeLabel = Label(master, textvariable=time[i])  #
        timeLabel.grid(row=i % 36, column=(i // 36) * 6 + 2, sticky=W, pady=2)  #
